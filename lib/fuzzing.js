/*
	* potential new mutators: special ints
*/

var rotate = require("bitwise-rotation");
...
var fuzz = {
  ...
  bitRotate: function(buff, opts) {
    if (!opts) opts = {};
    if (!opts.fuzz_factor) opts.fuzz_factor = 24;
    buff = new Buffer(buff, "binary");
    for (var i = 0; i < util.rand(buff.length / opts.fuzz_factor); i++) {
      var byte = buff[util.rand(buff.length)];
      // for each position decide rotate direction and amount
      if (util.rand(2)) var dir = "left";
      else var dir = "right";
      var shift = util.rand(9);
      if (dir == "left") var res = rotate.rolInt8(byte, shift);
      else var res = rotate.rorInt8(byte, shift);
      res = res.toString(16);
      buff[i] = res;
    }
    return buff.toString("binary");
