...
Fuzzer.utils.parseArgs = function(args) {
	...
	for (var i = 0; i < args.length; i++) {
      ...
      case "-x":
        // mutators to use
        mutators = args[++i].split(",");
        var map = {
          "bitflip": fuzz.seqBitFlip,
          "byteflip": fuzz.seqByteFlip,
          "buffmangler": fuzz.buffMangler,
          "chunkspew": fuzz.chunkSpew,
          "bytearith": fuzz.byteArithmetic,
          "bitrotate": fuzz.bitRotate,
          "ripple": fuzz.ripple
        }
        var active = [];
        for (var j = 0; j < mutators.length; j++) {
          if (map[mutators[j]] == undefined) {
            console.log("[!] %s is not a valid mutator(bitflip, byteflip, buffmangler, chunkspew, bytearith, bitrotate, ripple)", mutators[j]);
            process.exit();
          }
          active.push(map[mutators[j]]);
        }
        Fuzzer.fuzzMethods = active;
        args.mutators = active;
        continue;

      case "-p":
        // preserve case files
        args.preserve_cases = true;
        continue;
...
Fuzzer.utils.nextCase = function(res) {
  ...
  //console.log(Fuzzer.run.config.monitoring.mode);
	if (res) {
		...
		if (res == "normal_exit") {
			// log result and delete boring case if we're not in preserve mode
			console.log("[i] subject exited normally");
      if (!Fuzzer.run.opts.preserve_cases) {
  ...
  if (seed != "ngen") {
    ...
    buff = Fuzzer.fuzzMethods[method](buff, {
      step_floor: 8 * 8,
      step_ceil: 64 * 8,
      fuzz_factor: Fuzzer.run.opts.fuzz_factor
    });
  }
  ...
  switch (Fuzzer.run.config.type) {
    case "autofuzz":
      ...
      log.logEvent(
        ...
        "fuzzing " + Fuzzer.run.config.subject + " with " + name + 
        ", chose mutator " + Fuzzer.fuzzMethods[method].name + " and monitoring with " + Fuzzer.run.config.monitoring.mode
      );
