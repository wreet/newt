...
procmon.launchers = {
    ...
    if (monitor_opts.kill_after) {
      procmon.timers.push(
        setTimeout(function() {
          procmon.cmd.kill();
          //cb("normal_exit"); // let the normal exit handler deal with it
        }, monitor_opts.kill_after * 1000)
      ); // milliseconds
    } // end kill_after if

		switch (monitor_opts.type) {
			case "asan": // instrumented with asan/ubsan, good choice
				procmon.cmd = child.exec(cmd.join(" "), function(err, stdout, stderr) {
...
procmon.handlers = {
	...
	parseCmdResult: function(res, cb) {
		...
		try {
			...
			// seems we sometimes get here with a sig32
			return cb("not_exploitable");
		}
		switch (sig.signal) {
				// interestings
