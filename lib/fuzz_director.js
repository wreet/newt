...
Fuzzer.utils.nextCase = function(res) {
  ...
  if (seed.indexOf('.js') != -1) { // kind of yuck, I know
    console.log("it's an ngen file!");
  ...
  // we need to not do all the file i/o shit if we're generating
  if (seed == "ngen") {
    // since the ngenerator function blocks, buff should be ready by now

  }
  else { // we're doing file format mutation
